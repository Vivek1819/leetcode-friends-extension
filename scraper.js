...
const sendSubmissionsToBackend = async (username, submissions, checkpointSubmissionId = null) => {
  ...
  try {
    ...
    const latestSubmissionId = checkpointSubmissionId || 
...
const setupNavigationHandlers = (username) => {
  ...
  const initScraper = async () => {
    if (window.forceFullScan) {
      ...
      try {
        const response = await fetch(`${window.LeetCodeFriendsConfig.API_BASE_URL}/${username}/checkpoint`);
        const data = await response.json();
        
        if (data && data.lastScrapedSubmissionId) {
          lastScrapedSubmissionId = data.lastScrapedSubmissionId;
          console.log(`Retrieved checkpoint from database: ${lastScrapedSubmissionId}`);
        } else {
          console.log("No checkpoint found in database, will scrape all submissions");
        }
      } catch (error) {
        console.error("Error fetching checkpoint from backend:", error);
        console.log("Will scrape all submissions due to error fetching checkpoint");
      }
    ...
    try {
      ...
      const submissions = parseSubmissionsPage();      if (submissions.length > 0) {
        ...
        if (currentPage === 1 && submissions[0].submissionId) {
          ...
          console.log(`📍 Set latest submission ID as checkpoint: ${latestSubmissionId}`);
        }
        if (lastScrapedSubmissionId) {
          ...
          const foundPreviousSubmission = submissions.some(sub => {
            ...
            if (matches) {
              console.log(`Match found! Submission ${subId} matches checkpoint ${checkpointId}`);
            }
            return matches;
          });
        ...
        await sendSubmissionsToBackend(username, submissions, latestSubmissionId);
